...
async function main () {
    ...
    console.log('======================');
    console.log('Current state\'s price =', usdPrice(await deployedOffchainOracle.getRateToEth(token, true) / 10 ** (18 - decimals)));
    ...
    for (let i = 0; i < oracles.allOracles.length; i++) {
        ...
        const price = usdPrice(await offchainOracle.getRateToEth(token, true) / 10 ** (18 - decimals));
