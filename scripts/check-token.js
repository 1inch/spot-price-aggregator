...
async function main () {
    ...
    console.log('======================');
    console.log('Current state\'s price =', usdPrice(await deployedOffchainOracle.getRateToEth(token, true, tresholdFilter) / 10 ** (18 - decimals)));
    ...
    for (let i = 0; i < oracles.allOracles.length; i++) {
        ...
        const price = usdPrice(await offchainOracle.getRateToEth(token, true, tresholdFilter) / 10 ** (18 - decimals));
        if (parseFloat(price) !== 0) {
            ...
            for (let j = 0; j < connectors.length; j++) {
                ...
                try {
                    ...
                    const { rate, weight } = await oracle.getRate(token, weth, connectors[j], tresholdFilter);
