...
describe('MooniswapOracle', function () {
    ...
    it('ETH -> DAI', async function () {
        ...
        await testRate([tokens.ETH, tokens.WETH], tokens.DAI, tokens.NONE, mooniswapOracle, uniswapV3Oracle);
    });

    it('DAI -> ETH', async function () {
        const { mooniswapOracle, uniswapV3Oracle } = await loadFixture(initContracts);
        await testRate(tokens.DAI, [tokens.ETH, tokens.WETH], tokens.NONE, mooniswapOracle, uniswapV3Oracle);
    });

    it('ETH -> USDC', async function () {
        const { mooniswapOracle, uniswapV3Oracle } = await loadFixture(initContracts);
        await testRate([tokens.ETH, tokens.WETH], tokens.USDC, tokens.NONE, mooniswapOracle, uniswapV3Oracle);
    });
