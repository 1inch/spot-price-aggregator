...
const {
    tokens,
    deployParams: { AaveWrapperV2, Curve, Uniswap, UniswapV2 },
...
describe('CurveOracle', function () {
    ...
    it('usdt -> wbtc', async function () {
        ...
        const expectedRate = await uniswapV2LikeOracle.getRate(tokens.USDT, tokens.WBTC, tokens.NONE, thresholdFilter);
        assertRoughlyEqualValues(rate.rate.toString(), expectedRate.rate.toString(), '0.05');
    });

    it('wbtc -> usdc', async function () {
        const { curveOracle, uniswapV2LikeOracle } = await loadFixture(initContracts);
        const expectedRate = await uniswapV2LikeOracle.getRate(tokens.WBTC, tokens.USDT, tokens.NONE, thresholdFilter);
    ...
    it('wbtc -> weth', async function () {
        const { curveOracle, uniswapV2LikeOracle } = await loadFixture(initContracts);
        const expectedRate = await uniswapV2LikeOracle.getRate(tokens.WBTC, tokens.WETH, tokens.NONE, thresholdFilter);
...
describe('CurveOracle doesn\'t ruin rates', function () {
    ...
    it('WETH WBTC', async function () {
        const { oldOffchainOracle, newOffchainOracle } = await loadFixture(initContracts);
        await testRate(tokens.WETH, tokens.WBTC, oldOffchainOracle, newOffchainOracle);
    });
