...
contract OffchainOracle is Ownable {
    ...
    struct OracleTokenBlacklist {
        ...
        BlackListType blacklistType; // uint8
        uint248 pairCount; // packed with blacklistType into 1 slot
    }
    ...
    *         Automatically upgrades blacklistType NONE -> PAIR when adding first pair.
    *         Automatically resets blacklistType PAIR -> NONE when removing last pair.
    *         Does NOT touch ENTIRE_ORACLE — that token is already fully blocked.
    *         Idempotent: adding an already-added pair or removing an already-removed pair is a no-op.
    ...
    */
    function setOracleTokenBlacklistPair(address oracle, IERC20 token1, IERC20 token2, bool isBlacklistedPair) external onlyOwner {
        OracleTokenBlacklist storage entry = oracleTokenBlacklisted[oracle][token1];
        bool wasBlacklisted = entry.isBlacklistedTokens[token2];
        if (isBlacklistedPair && !wasBlacklisted) {
            entry.pairCount++;
            if (entry.blacklistType == BlackListType.NONE) {
                entry.blacklistType = BlackListType.PAIR;
                oracleBlacklistCount[oracle]++;
            }
        } else if (!isBlacklistedPair && wasBlacklisted) {
            entry.pairCount--;
            if (entry.pairCount == 0 && entry.blacklistType == BlackListType.PAIR) {
                entry.blacklistType = BlackListType.NONE;
                oracleBlacklistCount[oracle]--;
            }
        }
        entry.isBlacklistedTokens[token2] = isBlacklistedPair;
    ...
    */
    function getRateWithCustomConnectors(
        ...
        (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) = getRatesAndWeightsWithCustomConnectors(srcToken, dstToken, useWrappers, customConnectors, thresholdFilter);
    ...
    */
    function getRateToEthWithCustomConnectors(IERC20 srcToken, bool useSrcWrappers, IERC20[] memory customConnectors, uint256 thresholdFilter) public view returns (uint256 weightedRate) {
        (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) = getRatesAndWeightsToEthWithCustomConnectors(srcToken, useSrcWrappers, customConnectors, thresholdFilter);
        if (wrappedPrice > 0) return wrappedPrice;
        (weightedRate,) = ratesAndWeights.getRateAndWeightWithSafeMath(thresholdFilter);
    }
    ...
    */
    function getRatesAndWeightsToEthWithCustomConnectors(IERC20 srcToken, bool useSrcWrappers, IERC20[] memory customConnectors, uint256 thresholdFilter) public view returns (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) {
        if(thresholdFilter >= 100) revert TooBigThreshold();
